...
//    printf("threadis ready to run= %d\n", thread->GetPID());

    if(schedAlgo == 2)
        readyThreadList->SortedInsert((void *)thread,burstPredict[thread->GetPID()]);
    else if(schedAlgo >=7)
        return;
    else
...
NachOSscheduler::FindNextThreadToRun ()
{
  //  printf("\nFindNextThreadToRun: %d\n", currentThread->GetPID());
    if(schedAlgo >= 7)
    {
      ...
      {
        if(!exitThreadArray[i] && threadBasePriority[i] != 0 && threadArray[i]->getStatus() == READY)
        {
          if(threadPriority[i] < minPriority)
          {
            minPriority = threadPriority[i];
            nextThread = threadArray[i];
          }
        }
      }
    ...
    {
      //NachOSThread *ptr;
      //ptr = (NachOSThread *)readyThreadList->Remove();
      //if(ptr != NULL)
      //printf("The thread removed from list is %d at t=%d\n", ptr->GetPID(),stats->totalTicks );
...
NachOSscheduler::Schedule (NachOSThread *nextThread)
{
    ...
    if(currentThread->GettotalCPUburst() == 0) {
