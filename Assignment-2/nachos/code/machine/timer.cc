...
Timer::TimeOfNextInterrupt()
{
    ...
    {
      if(schedAlgo > 2 && currentThread != NULL)
      {
          if( sleepQueueHead != NULL && sleepQueueHead->GetThread() != NULL )
          {
              if(currentThread->getStatus() == BLOCKED)     // currentThread went to sleep (CPU is idle)
              {
                doYield = true;
                int wakeTime = sleepQueueHead->GetWhen() - stats->totalTicks;
                //printf("TimeOfNextInterrupt: PID=%d  stat %d prevstime %d\n", currentThread->GetPID(), stats->totalTicks, currentThread->GetprevstartTime());
                if(wakeTime > 0 && wakeTime < TimerTicks)
                  return wakeTime;                        // Optimizing sleepTime
                else
                  return TimerTicks;
              }
          }

          int currentBurstLen = stats->totalTicks - currentThread->GetprevstartTime();
    //      printf("TimeOfNextInterrupt: PID=%d burstlen = %d stat %d prevstime %d\n", currentThread->GetPID(), currentBurstLen, stats->totalTicks, currentThread->GetprevstartTime());
          if(currentBurstLen >= TimerTicks)   // Burst completed
          {
            doYield = true;
            return TimerTicks;
          }
          else                                // Burst not completed yet
          {
            doYield = false;
            return (TimerTicks - currentBurstLen);
          }
      }
      else
          return TimerTicks;
