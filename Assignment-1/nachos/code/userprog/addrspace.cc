...
// how big is address space?
	...
	if(MaxPage + numPagesInVM > NumPhysPages)
					MemoryFull = TRUE;
...
NachOSpageTable[i].physicalPage = MaxPage++;
...
executable->ReadAt(&(machine->mainMemory[NachOSpageTable[0].physicalPage*PageSize+noffH.code.virtualAddr]),noffH.code.size, noffH.code.inFileAddr);
	}
	if (noffH.initData.size > 0) {
			DEBUG('a', "Initializing data segment, at 0x%x, size %d\n",
		noffH.initData.virtualAddr, noffH.initData.size);

executable->ReadAt(&(machine->mainMemory[NachOSpageTable[0].physicalPage*PageSize+noffH.initData.virtualAddr]),noffH.initData.size, noffH.initData.inFileAddr);
	}

}

//----------------------------------------------------------------------
//ProcessAddrSpace:: CopyContents
// 	Create an address space to run a user program.
//	copies contents of currently active
//	process into it
//----------------------------------------------------------------------

ProcessAddrSpace::ProcessAddrSpace()
{
TranslationEntry *table = machine->NachOSpageTable;
numPagesInVM =  machine->pageTableSize;
unsigned int start_read = (unsigned)(table[0].physicalPage)*PageSize;
unsigned int start_write = (unsigned)(MaxPage)*PageSize;
unsigned int length = (unsigned)numPagesInVM*PageSize;
unsigned int end = start_read + length;

//check if enough memory is available for allocation
if(end + length > MemorySize) MemoryFull = TRUE;
else{
		//initaialize table
		NachOSpageTable = new TranslationEntry[numPagesInVM];
		unsigned int i;
		//copy all the page table entries
		for (i = 0; i < numPagesInVM; i++) {

		     		NachOSpageTable[i].virtualPage = table[i].virtualPage;
		        NachOSpageTable[i].physicalPage = MaxPage++;
		        NachOSpageTable[i].valid = TRUE;
		        NachOSpageTable[i].use = FALSE;
		        NachOSpageTable[i].dirty = FALSE;
		...
		for (i = start_read ; i < end; i++) {
		        machine->mainMemory[start_write++] = machine->mainMemory[i];
		 }
}
