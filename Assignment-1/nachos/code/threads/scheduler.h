...
class NachOSscheduler {
  public:
    ...
    void InsertSleepingThread(NachOSThread* thread);
          //Insert a thread passed as argument in sleepingThreadList
    NachOSThread* RemoveSleepingThread(NachOSThread* thread);
          //remove thread passed as argument from sleepingThreadList
    bool IsSleepingListEmpty();
  private:
    List *readyThreadList;  		// queue of threads that are ready to run,
				// but not running
    List *sleepingThreadList;   //queue of threads that are in sleep state
};
